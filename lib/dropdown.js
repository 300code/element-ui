...
/* harmony default export */ var dropdownvue_type_script_lang_js = ({
  ...
  mounted: function mounted() {
    if (this.raw && this.$slots.dropdown) {
      var menuVNode = this.$slots.dropdown[0];
      ...
      instance.$options.attrs = _extends({}, menuVNode.data.attrs);
      instance.$vnode = menuVNode;

      // 3. NOW mount it
      instance.$mount();

      var _menuVNode$data = menuVNode.data,
          staticClass = _menuVNode$data.staticClass,
          attrs = _menuVNode$data.attrs;

      if (staticClass) instance.$el.className += ' ' + staticClass;
      if (attrs) {
        Object.keys(attrs).forEach(function (key) {
          return instance.$el.setAttribute(key, attrs[key]);
        });
      }
  ...
  render: function render(h) {
    ...
    if (this.raw) {
      ...
      vnode.data.directives = [{ name: 'clickoutside', rawName: 'v-clickoutside', value: hide, expression: 'hide', def: clickoutside_default.a }];
      vnode.data.attrs = _extends({}, vnode.data.attrs, { 'aria-disabled': disabled, role: 'button' });

      var activeClasses = { 'is-active': this.visible };
