...
/* harmony default export */ var dropdownvue_type_script_lang_js = ({
  ...
  watch: {
    visible: function visible(val) {
      ...
      if (this.$el) {
        if (val) this.$el.classList.add('is-active');else this.$el.classList.remove('is-active');
      }
    },
  ...
  methods: {
    ...
    hide: function hide() {
      ...
      if (target) {
        var isPopup = target.closest('.el-dialog__wrapper') || target.closest('.el-message-box__wrapper') || target.closest('.v-modal'); // The dark backdrop
      ...
      clearTimeout(this.timeout);
      this.timeout = setTimeout(function () {
        if (_this3.closeLocked) return;
        _this3.visible = false;
        console.log('setTimeout', _this3.visible);
    ...
    initEvent: function initEvent() {
      ...
      this.triggerElm = splitButton ? this.$refs.trigger.$el
  ...
  render: function render(h) {
    ...
    if (this.raw && this.$slots.dropdown) {
      var vnode = triggerElm[0];
      vnode.data = vnode.data || {};

      // attrs & class
      vnode.data.attrs = _extends({}, vnode.data.attrs, { 'aria-disabled': disabled, role: 'button' });
      vnode.data.class = [vnode.data.class, { 'is-active': this.visible }];

      // hooks
      vnode.data.hook = vnode.data.hook || {};
      var oldInsert = vnode.data.hook.insert;

      vnode.data.hook.insert = function (vnodeElm) {
        var triggerEl = vnodeElm.elm;
        var menuVNode = _this6.$slots.dropdown[0];

        if (menuVNode && menuVNode.componentOptions) {
          var MenuCtor = menuVNode.componentOptions.Ctor;
          var menuInstance = new MenuCtor({
            propsData: menuVNode.componentOptions.propsData,
            parent: _this6
          });
          ...
          _this6.popperElm = menuInstance.$el;
          ...
          if (_this6.dropdown && _this6.dropdown.popperElm) {
            ...
            var parentPopper = _this6.dropdown.popperElm;
          ...
          clickoutside_default.a.bind(triggerEl, {
            value: _this6.hide.bind(_this6),
            expression: 'hide'
          }, vnodeElm);

          // 4️⃣ Sync the visibility once to start
          menuInstance.$emit('visible', _this6.visible);

          // 5️⃣ Manual listener for the Menu items to respect hideOnClick
          menuInstance.$on('menu-item-click', function (command, instance) {
            _this6.handleMenuItemClick(command, instance);
          });

          _this6.initDomOperation();
        }
      };

      vnode.data.hook.destroy = function (vnodeElm) {
        clickoutside_default.a.unbind(vnodeElm.elm);
      };
...
var dropdown_render, staticRenderFns
